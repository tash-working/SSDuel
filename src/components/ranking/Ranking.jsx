    ...
    {users.map((user, index) => (
      <div key={index} className="bg-white p-2 rounded-lg my-2 shadow-md">
        <div className="flex items-center justify-between">
          <h2 key={index} className="text-lg font-medium">{index + 1}: {user.userName}</h2>
          <h3 className="text-green-500 font-semibold">{user.like} Likes</h3>
        </div>
        <hr className="border-b border-b-2 border-gray-300 my-4 mx-auto opacity-50" />
        <div className="flex items-center justify-between">
          <h3 className="text-gray-500 font-medium">Rejections: {user.total - user.like}</h3>
        </div>
      </div>
    ))}
  </div>
